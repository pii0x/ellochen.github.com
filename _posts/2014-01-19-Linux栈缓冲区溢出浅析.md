...
程序开始执行的时候,  有一个叫做EIP的小伙子, 啊不, 是寄存器, EIP存储着下一个要执行的指令的地址, 也就是上图中Text区域中的某个位置, 系统会忠实地依次执行EIP指向的指令, 在执行过程中如果遇到函数调用, 在汇编这个层级, 一般是遇到call指令的时候, 栈的show time就开始了, 也就是上图中Stack的那个区域. 我们都知道实现递归的传统方式就是依靠栈, 有了栈的存在, 函数的层层调用就容易的多, 调用函数的时候, 会先把传给该函数的参数压入栈中, 将函数返回时下一个指令的地址压入栈中我们简称保存该指令的区域为RET, 然后抽象的角度来看会为该函数创建一个栈帧, 会向栈帧中压入函数调用者栈帧的地址, 然后为该函数中定义的各个变量依次预留空间, 无论该函数多么复杂, 定义了多少变量, 当它执行完后首先会将该函数栈帧弹栈然后通过ret指令来将控制权交给函数调用者, 并执行之前保存的RET指向的指令, 函数调用的控制权转换就这样轻易的实现, 这都是栈的功劳, 我们再次以图形的角度看看函数调用时栈里的内存布局:  
...
对应的汇编代码如下:  
	...
	0x08048430 <+12>:	cmpl   $0x1,0x8(%ebp)
	0x08048434 <+16>:	jne    0x8048458 <main+52>
	0x08048436 <+18>:	mov    0xc(%ebp),%eax
	0x08048439 <+21>:	mov    (%eax),%edx
	0x0804843b <+23>:	mov    $0x8048560,%eax
	0x08048440 <+28>:	mov    %edx,0x4(%esp)
	0x08048444 <+32>:	mov    %eax,(%esp)
	0x08048447 <+35>:	call   0x8048320 <printf@plt>
	0x0804844c <+40>:	movl   $0x1,(%esp)
	0x08048453 <+47>:	call   0x8048350 <exit@plt>
	0x08048458 <+52>:	mov    0xc(%ebp),%eax
	0x0804845b <+55>:	add    $0x4,%eax
	0x0804845e <+58>:	mov    (%eax),%eax
	0x08048460 <+60>:	mov    %eax,0x4(%esp)
	0x08048464 <+64>:	lea    0x10(%esp),%eax
	0x08048468 <+68>:	mov    %eax,(%esp)
	0x0804846b <+71>:	call   0x8048330 <strcpy@plt>
	0x08048470 <+76>:	mov    $0x8048574,%eax
	0x08048475 <+81>:	lea    0x10(%esp),%edx
	0x08048479 <+85>:	mov    %edx,0x4(%esp)
	0x0804847d <+89>:	mov    %eax,(%esp)
	0x08048480 <+92>:	call   0x8048320 <printf@plt>
	0x08048485 <+97>:	mov    $0x0,%eax
