...
3. **有0，1，...,n-1这n个数排成一个圆圈，从数字0开始每次从这个圆圈里删除第m个数。求出这个圆圈里剩下的最后一个数字。**  
	...
	这个美妙的解法是怎么回事呢，显然不是我想出来的，我是在《剑指offer》上看到的，有一天在《具体数学》上也看到了，《具体数学》上貌似并没给出一般性的解法只给出了m = 2的情形。下面说说我的理解：  
	先按步骤思考归纳：
	1. 初始的序列是0，1，2，...,n-1，序列每个元素对应的下标是0，1，2，...,n-1。
	2. 删除第m个数，第m个数的下标应该是k = (m - 1) % n,为何要不是m-1而要取余？因为m-1可能大于n。
	3. 第2步之后，起始数由0变成了k + 1，形成新的序列k+1,k+2,k+3,...,n-1,0,1,...,k-2,k-1，其对应的下标是0，1，2，...,n-k-2,n-k-1,n-k,...,n-3,n-2。从下标到值的映射关系为,y=(x+k+1)%n。
	4. 我们设求最终剩余数的下标函数为p(n,m)，根据3）可知，求最终剩余数的函数为q(n,m)=(p(n,m)+k+1)%n。将k = (m - 1) % n带入上式得q(n,m)=(p(n,m)+m)%n。
